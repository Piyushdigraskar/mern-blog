...
export const updateUser = async(req, res, next)=>{
    ...
    if(req.body.username){
        ...
        if(req.body.username !== req.body.username.toLowerCase()){
            return next(errorHandler(400, 'Username cannot be Lowercase'));
        }
    ...
    export const deleteUser = async(req, res, next)=>{
        if(req.user.id !== req.params.userId){
            return next(errorHandler(403, 'You are not allowed top delete this user'));
        }
        try {
    ...
    export const signout = (req, res)=>{
        try {
            res.clearCookie('access-token').status(200).json('User has been successfully signed out');
        } catch (error) {
            next(error);
        }
    }